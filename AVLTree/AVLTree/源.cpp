...
//LL   4个旋转均返回旋转后的根节点值   具体情形看攻略那4*2=8的集合图
template<class T>
AVLTreeNode<T>* AVLTree<T>::leftLeftRotation(AVLTreeNode<T>* k)
{
	AVLTreeNode<T>* k1;
	k1 = k->left; k->left = k1->right;
	k1->right = k;

	k->height = max(height(k->left), height(k->right)) + 1;//LL RR不要忘记自己更改height值！！
...
//RR
template<class T>
AVLTreeNode<T>* AVLTree<T>::rightRightRotation(AVLTreeNode<T>* k)
{
	AVLTreeNode<T>* k2;
	k2 = k->right;
	k->right = k2->left;
	...
	k->height = max(height(k->left), height(k->right)) + 1;//同上
...
//LR
template<class T>
AVLTreeNode<T>* AVLTree<T>::leftRightRotation(AVLTreeNode<T>* k)//LR  RL就不必自己更改height值了，递归已经更改好了
{
	k->left = rightRightRotation(k->left);
	return leftLeftRotation(k);//这里的输入参数是k！！不是k->left！！总爱写错
}

//RL
template<class T>
AVLTreeNode<T>* AVLTree<T>::rightLeftRotation(AVLTreeNode<T>* k)
{
	k->right = leftLeftRotation(k->right);
	return rightRightRotation(k);//同上
}
...
AVLTreeNode<T>* AVLTree<T>::remove(AVLTreeNode<T>* &tree, AVLTreeNode<T>* z)
{
	...
	if (z->key < tree->key)
	{
		...
		if (height(tree->right) - height(tree->left) == 2)
		{
			...
			if (height(r->left) > height(r->right))//和insert不同，这里没有key值给你用，用height进行比较，看图就能明白
				tree = rightLeftRotation(tree);
			else
				tree = rightRightRotation(tree);
		}
